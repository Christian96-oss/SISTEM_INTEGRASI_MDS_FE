...
const OrderDetails = () => {
  ...
  const isCurrentUser = user.email !== orderDetails.creator.email;
    ...
    <Box borderRadius="15px" padding="20px" bgcolor="#FCFCFC" width="fit-content">
      ...
      <Box mt="20px" display="flex" flexDirection={{ xs: "column", lg: "row" }} gap={4}>
        <Box flex={1} maxWidth={764}>
          <img src={orderDetails.photo} alt="order_details-img" height={546} style={{ objectFit: "cover", borderRadius: "10px" }} className="order_details-img" />
        ...
        <Box width="100%" flex={1} maxWidth={326} display="flex" flexDirection="column" gap="20px">
          <Stack width="100%" p={2} direction="column" justifyContent="center" alignItems="center" border="1px solid #E4E4E4" borderRadius={2}>
            <Stack mt={2} justifyContent="center" alignItems="center" textAlign="center">
              <img
                src={checkImage(orderDetails.creator.avatar) ? orderDetails.creator.avatar : "https://upload.wikimedia.org/wikipedia/commons/thumb/5/59/User-avatar.svg/2048px-User-avatar.svg.png"}
                alt="avatar"
                width={90}
                height={90}
                style={{
              ...
              <Typography mt={1} fontSize={16} fontWeight={600} color="#11142D">
                {orderDetails.creator.allOrder.length} Order
              </Typography>
            </Stack>
            ...
            <Stack width="100%" mt="25px" direction="row" flexWrap="wrap" gap={2}>
              <CustomButton
                ...
                handleClick={() => {
                  if (isCurrentUser) {
                    navigate(`/order/edit/${orderDetails._id}`);
                  }
                }}
              />
              <CustomButton
          ...
          {/* <Stack>
            <img src="https://serpmedia.org/scigen/images/googlemaps-nyc-standard.png?crc=3787557525" width="100%" height={306} style={{ borderRadius: 10, objectFit: "cover" }} />
          </Stack> */}
          <Box>
